    ...
    if (stage==0) {
        ...
        if (par("avoidBeaconSynchronization").boolValue() == true) {
            ...
            if (sendEntMsg) {
                ...
                //FIXME vereificar a questao de cooordenar a fatia de tempo transmitindo no canal
            }
...
void VehApp::populateWSM(WaveShortMessage* wsm, int rcvId, int serial) {
    ...
    else if (EntertainmentMessageA* esmA = dynamic_cast<EntertainmentMessageA*>(esmA)) {
        esmA->setChannelNumber(Channels::SCH1);
        esmA->addBitLength(1000);//FIXME need to be modified bby the video frame size
        esmA->setUserPriority(4);
    }
    else if (EntertainmentMessageB* esmB = dynamic_cast<EntertainmentMessageB*>(esmB)) {
        esmA->setChannelNumber(Channels::SCH1);
        esmA->addBitLength(1000);//FIXME need to be modified bby the video frame size
        esmA->setUserPriority(4);
...
void VehApp::handleSelfMsg(cMessage* msg) {
    ...
    case SEND_ENT_A_EVT: {
        ...
        populateWSM(entMsgA);
        sendDown(entMsgA);
        scheduleAt(simTime() + entMsgAInterval, sendEntMsgAEvt); // FIXME this will need to be scheduled dynamically by the time found in the video
        break;
    }
    case SEND_ENT_B_EVT: {
        ...
        populateWSM(entMsgB);
        sendDown(entMsgB);
        scheduleAt(simTime() + entMsgBInterval, sendEntMsgBEvt); // FIXME this will need to be scheduled dynamically by the time found in the video
        break;
    }
    default: {
        if (msg)
